...
### 3. Sıkı API ve Tool Call ID Uyumluluğu
- DeepSeek ve diğer bazı strict (katı) LLM sağlayıcıları, bir sohbet geçmişinde `tool` rolündeki her mesajın `tool_call_id` değerinin, hemen öncesindeki `assistant` mesajının `tool_calls` dizisindeki ilgili çağrının `id` değeriyle birebir eşleşmesini ve bunların `null` veya boş olmamasını şart koşar.
- Hata almayı önlemek için, geçmiş yüklenirken eksik veya `null` olan `tool_call_id` ve `id` değerleri dinamik olarak üretilip eşleştirilerek normalize edilir.
- Canlı sohbet turlarında da eğer LLM proxy'si (örn. OpenCode gibi) boş/geçersiz `tc.id` dönerse, sistem dinamik olarak benzersiz kimlikler üreterek uyumluluğu korur.
...
## Tool Geçmişi 500 Koruması

- Eski oturumlarda kayıtlı `tool_calls` / `tool` mesajları, yeni LLM isteğine doğrudan native tool mesajı olarak taşınmaz; `append_chat_history_for_llm()` tarafından asistan bağlam metnine dönüştürülür. Böylece DeepSeek/OpenCode gibi strict sağlayıcılarda `missing field tool_call_id` hatası sohbeti 500'e düşürmez.
