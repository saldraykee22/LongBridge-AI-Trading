...
- **Haber Duyarlılık Token Limiti (Düzeltildi):** `analyze_news_sentiment()` fonksiyonunda `max_tokens` 1500'den 2500'e çıkarılmıştır. Model token limitine ulaşıp JSON'u tam döndüremeden kestiği için `extract_json` parse hatası alıyordu.
- **Kesilmiş JSON Tamamlama (Yeni):** `extract_json()` fonksiyonuna 3. katman eklendi. Model token limiti nedeniyle JSON'u keserse, fonksiyon otomatik olarak açık string'leri kapatır ve eksik array/object kapanışlarını (`]`, `}`) ekleyerek kısmi veriyi kabul eder.

---

## Analiz 500 Koruması

- `get_stock_analysis()` içinde LLM JSON parse, boş yanıt, kota veya servis hatası ikinci denemede de düzelmezse endpoint 500/502/422 ile boş dönmez. Mevcut fiyat, aylık değişim, RSI, hacim, temel metrikler ve haber özetinden deterministik bir yedek analiz JSON'u üretilir; `score`, `strategy` ve `agents` alanları her zaman ön yüzün beklediği formatta kalır.
